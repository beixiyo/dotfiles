...
- 使用 `apply_patch`、Write 或 Edit 修改代码后，`.claude/hooks/post-write-code.ts` 可能继续自动修改刚写入的文件
- 因此不要只依据 patch 文本判断最终内容，后续操作和行为判断应以 hook 完成后的磁盘文件为准
...
- 同类模块优先使用文件夹组织。由 `index` / `init` 统一导出(仅针对 js、lua 等)
...
- 获取 Web 内容时优先使用 MCP、WebFetch 或已有内容；需要把网页转为 markdown 使用：
```bash
