-- ╭───────────────────────────────────────────────────────────────╮
-- │     native 模式（WezTerm 原生 tab + pane，集成 vv-splits）    │
-- ╰───────────────────────────────────────────────────────────────╯
-- 机制：检测前台进程是否为 nvim，是则透传键给 nvim（vv-splits 接管），
...
-- Ctrl+Alt+h/j/k/l：Nvim 内交给 vv-splits，否则切换 WezTerm 窗格
...
-- Ctrl+Alt+方向键：Nvim 内交给 vv-splits，否则调整窗格大小
...
function M.apply(config)
  ...
  local keys = {
    ...
    -- ── 分屏导航（Nvim 内透传给 vv-splits，否则切 WezTerm pane）──
    nav_or_pane('Left', 'h'),
    nav_or_pane('Down', 'j'),
    nav_or_pane('Up', 'k'),
    nav_or_pane('Right', 'l'),
